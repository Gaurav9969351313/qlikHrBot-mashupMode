...
export class ConfigMasterComponent implements OnInit {
  ...
  initialize() {
    this.sharedService.getMasterConfig().subscribe((config: any) => {
      this.appConfigRows = config.appLevelConfig;
      // console.log("Config Received In App componenet", config)
    });
  }
  ...
  findSelectedAppConfigIndex() {
    return this.appConfigRows.indexOf(this.selectedAppConfigRow);
  }
